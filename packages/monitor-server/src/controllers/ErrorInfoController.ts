 ...
 * @param ip 客户端IP
 */
const handleSingleErrorInfo = async (ctx, errorData: any) => {
  // 构造完整的错误信息
  const errorInfo: IErrorInfo = {
    ...errorData,
    error_info: JSON.stringify(errorData.extraData) || ''
  };

  // 保存到数据库
  const result = await ErrorInfoModel.create(errorInfo);
 ...
 * @param ip 客户端IP
 */
const handleBatchErrorInfo = async (ctx, errorsData: any[]) => {
  ...
  // 构造完整的错误信息数组
  const errors: IErrorInfo[] = errorsData.map(errorData => ({
    ...errorData,
    error_info: JSON.stringify(errorData.extraData) || ''
  }));

  // 批量保存到数据库
  await ErrorInfoModel.createBatch(errors);
 ...
 * @param ctx Koa上下文
 */
export const handleErrorInfo = async (ctx, reportInfo) => {
  try {

    if (Array.isArray(reportInfo)) {
      await handleBatchErrorInfo(ctx, reportInfo);
    } else {
      await handleSingleErrorInfo(ctx, reportInfo);
    }
  } catch (error) {
 ...
 * @param ctx Koa上下文
 */
export const getErrorInfos = async (ctx) => {
  try {
    ...
    // 查询数据
    const errors = await ErrorInfoModel.findAll({
      where,
      limit: limitNum,
      offset,
      order: [['id', 'DESC']]
    ...
    // 查询总数
    const total = await ErrorInfoModel.count({ where });
 ...
 * @param ctx Koa上下文
 */
export const getErrorInfoById = async (ctx) => {
  try {
    const { id } = ctx.params;
    ...
    const error = await ErrorInfoModel.findById(idNum);
 ...
 * @param ctx Koa上下文
 */
export const deleteErrorInfo = async (ctx) => {
  try {
    const { id } = ctx.params;
    const idNum = parseInt(id, 10);
    ...
    const result = await ErrorInfoModel.deleteById(idNum);
